...
// 给用户发送消息
func SendUserLoc(appId uint32, userId string, msgId string, message g.Map) (sendResults bool, err error) {
	...
	//server := model.NewServer(info.AccIp, info.AccPort)
	//msg, err := grpcclient.SendMsg(server, msgId, appId, userId, model.MessageCmdMsg, model.MessageCmdMsg, message)
	//if err != nil {
	//	fmt.Println("给用户发送消息失败", key, err)
	//
	//	return false, err
...
// 给本机用户发送消息
func SendUserMessageLocal(appId uint32, userId string, data g.Map) (sendResults bool, err error) {
	...
	// 发送消息
	client.SendMsg(gconv.Bytes(data))
	sendResults = true
